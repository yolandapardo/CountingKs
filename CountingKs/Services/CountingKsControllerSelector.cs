    ...
    public class CountingKsControllerSelector : DefaultHttpControllerSelector
    {
        ...
        public override HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
           ...
           else if(controllers.TryGetValue(controllerName, out descriptor))
            {
                ...
                //  var version = GetVersionFromHeaderVersion(request);
        ...
        private string GetVersionFromMediaType(HttpRequestMessage request)
        {
            ...
            foreach(var mime in accept)
            {
                ...
                if(match!=null)
                {
                    return match.Groups[2].Value; // Groups[0] es la expresion completa q match, Groups[1] la expresion q match con lo q esta en el primer parentesis, Groups[2] en el segundo parentesis
                }
            }
            return "1";
        }

        private string GetVersionFromHeaderVersion(HttpRequestMessage request)
        {
            var accept = request.Headers.Accept;
            foreach(var mime in accept)
            {
